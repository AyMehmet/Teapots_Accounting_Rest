...
public class InvoiceProductServiceImpl implements InvoiceProductService {
    ...
    public InvoiceProductServiceImpl(InvoiceProductRepository invoiceProductRepository, MapperUtil mapper, @Lazy InvoiceService invoiceService) {
...
////    }
////
////    public Map<LocalDateTime, BigDecimal> invoiceMonthlyProfitLost2() {
////        Map<LocalDateTime, BigDecimal> profitMap = new HashMap<>();
////        Company company = mapper.convert(securityService.getLoggedInCompany(), new Company());
////        invoiceProductRepository.findAllByProductCategoryCompany(company).stream()
////                .filter(invoiceProduct -> invoiceProduct.getProfitLoss() != BigDecimal.ZERO)
////                .collect(Collectors.groupingBy((InvoiceProduct::getLastUpdateDateTime),Collectors.reducing(BigDecimal.ZERO,InvoiceProduct::getProfitLoss,BigDecimal::add)))
////                .forEach(profitMap::put);
////        return profitMap;
////
////    }
//
////    public Map<String, BigDecimal> invoiceMonthlyProfitLost2() {
////        //Map<LocalDateTime, BigDecimal> profitMap = new HashMap<>();
////        Company company = mapper.convert(securityService.getLoggedInCompany(), new Company());
////        return getProfitListofInvoiceProduct().stream()
////                .map(invoiceProduct -> {
////                    invoiceProduct.getLastUpdateDateTime().getMonth()
////                })
////                        .collect(Collectors.groupingBy(inv->inv.).toList())
////
////
////
////
...
////    }
//
//    public List<InvoiceProduct>  getProfitListofInvoiceProduct(){
//        Company company = mapper.convert(securityService.getLoggedInCompany(), new Company());
//        return invoiceProductRepository.findAllByProductCategoryCompany(company).stream()
//                .filter(invoiceProduct -> invoiceProduct.getProfitLoss() != BigDecimal.ZERO)
//                .collect(Collectors.toList());
